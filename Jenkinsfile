...
def checkoutAndStash() {
    withCredentials([string(credentialsId: 'github-token', variable: 'GIT_TOKEN')]) {
        checkout scm: [
            ...
            userRemoteConfigs: [[url: "https://$GIT_TOKEN@github.com/festcloud/${REPO}.git"]]
        ]
...
def buildFolder() {
    ansiColor('xterm') {
        ...
        set -x
        mvn install -DskipTests -Drat.skip=true -Dcheckstyle.skip -P-submodules -D maven.repo.local=/root/.m2/repository -Duser.home=/root/
        pwd
        ls -la
